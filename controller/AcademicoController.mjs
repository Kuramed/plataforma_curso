...
export function salvarCategoria() {
    try {
        const inputTitulo = document.getElementById('cat-titulo');
    ...
    container.innerHTML = modulos.length === 0 ? '<p class="text-muted small">Nenhum módulo criado.</p>' : modulos.map(m => `
        <div class="border rounded p-3 mb-3 bg-light shadow-sm">
            <div class="d-flex justify-content-between align-items-center mb-2">
                ...
                <div>
                    <button class="btn btn-sm btn-link text-danger text-decoration-none" onclick="excluirItem('tb_modulos', '${m.id_modulo}', 'id_modulo', 'renderAdminModulosAulas')">Eliminar Módulo</button>
                    <button class="btn btn-sm btn-success ms-2" onclick="prepararNovaAula('${m.id_modulo}')">+ Adicionar Aula</button>
                </div>
            </div>
            <div class="ms-3 bg-white p-2 rounded border">
                ${aulas.filter(a => a.id_modulo === m.id_modulo).map(a => `
                    <div class="d-flex justify-content-between align-items-center py-1 border-bottom">
                        ...
                        <button class="btn btn-sm btn-link text-danger p-0" onclick="excluirItem('tb_aulas', '${a.id_aula}', 'id_aula', 'renderAdminModulosAulas')">Remover Aula</button>
                    </div>
